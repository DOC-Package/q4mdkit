...
def main():
    ...
    parser.add_argument('--segment', type=float, default=100.0,
                        help='Segment length in ps (default: 20.0)')
    parser.add_argument('--corr', type=float, default=50.0,
    ...
    # ==========================================================================
    # Cross-correlation analysis for top 6 molecules by λ
    # ==========================================================================
    n_top = min(6, n_cols)
    top_indices = np.argsort(np.abs(lambda_all))[::-1][:n_top]
    top_names = [col_names[i] for i in top_indices]
    top_lambdas = [lambda_all[i] for i in top_indices]
    
    print(f"\n=== Cross-correlation analysis (top {n_top} by λ) ===")
    print(f"Selected molecules: {', '.join(top_names)}")
    
    # Compute cross-correlation for all pairs
    n_pairs = n_top * (n_top - 1) // 2
    cross_corr_matrix = np.zeros((n_top, n_top, corr_length))
    J_cross_matrix = np.zeros((n_top, n_top, len(nu_out)))
    lambda_cross_matrix = np.zeros((n_top, n_top))
    
    print(f"\nComputing {n_pairs} cross-correlations...")
    for ii, i in enumerate(top_indices):
        for jj, j in enumerate(top_indices):
            if ii == jj:
                # Autocorrelation (already computed)
                t_corr, C, _ = compute_correlation_segmented(deltas[:, i], dt, segment_length, corr_length)
                cross_corr_matrix[ii, jj, :] = C
                J_cross_matrix[ii, jj, :] = J_all[:, i]
                lambda_cross_matrix[ii, jj] = lambda_all[i]
            else:
                # Cross-correlation
                t_corr, C_cross, _ = compute_cross_correlation_segmented(
                    deltas[:, i], deltas[:, j], dt, segment_length, corr_length)
                cross_corr_matrix[ii, jj, :] = C_cross
                J_cross = correlation_to_spectral_density(t_corr, C_cross, nu_out, T)
                J_cross_matrix[ii, jj, :] = J_cross
                lambda_cross_matrix[ii, jj] = C_cross[0] / (2 * kBT)
    
    # Print cross-correlation λ matrix
    print(f"\nCross-correlation λ matrix (cm⁻¹):")
    print(f"{'':>12}", end='')
    for name in top_names:
        print(f"{name:>12}", end='')
    print()
    for ii, name_i in enumerate(top_names):
        print(f"{name_i:>12}", end='')
        for jj in range(n_top):
            print(f"{lambda_cross_matrix[ii, jj]:>12.2f}", end='')
        print()
    
    # Print cross-correlation λ matrix in eV
    print(f"\nCross-correlation λ matrix (eV):")
    print(f"{'':>12}", end='')
    for name in top_names:
        print(f"{name:>12}", end='')
    print()
    for ii, name_i in enumerate(top_names):
        print(f"{name_i:>12}", end='')
        for jj in range(n_top):
            print(f"{lambda_cross_matrix[ii, jj]*cm_to_eV:>12.4f}", end='')
        print()
    
    # Plot cross-correlation spectral densities
    fig, axes = plt.subplots(n_top, n_top, figsize=(3*n_top, 3*n_top))
    
    vmax = np.max(np.abs(J_cross_matrix))
    
    for ii in range(n_top):
        for jj in range(n_top):
            ax = axes[ii, jj]
            J = J_cross_matrix[ii, jj, :]
            
            if ii == jj:
                # Autocorrelation - blue
                ax.plot(nu_out, J, 'b-', linewidth=1)
                ax.fill_between(nu_out, 0, J, alpha=0.3)
            else:
                # Cross-correlation - red for positive peak, green for negative
                ax.plot(nu_out, J, 'k-', linewidth=1)
                ax.fill_between(nu_out, 0, J, where=(J > 0), color='red', alpha=0.3)
                ax.fill_between(nu_out, 0, J, where=(J < 0), color='green', alpha=0.3)
                ax.axhline(y=0, color='gray', linestyle='-', linewidth=0.5)
            
            ax.set_xlim(0, nu_max)
            ax.set_ylim(-vmax*0.3, vmax*1.1)
            
            if ii == 0:
                ax.set_title(f'{top_names[jj]}', fontsize=10)
            if jj == 0:
                ax.set_ylabel(f'{top_names[ii]}', fontsize=10)
            if ii == n_top - 1:
                ax.set_xlabel(r'$\omega$ (cm⁻¹)', fontsize=9)
            
            # Show λ value
            ax.text(0.95, 0.95, f'λ={lambda_cross_matrix[ii,jj]*cm_to_eV:.3f}',
                    transform=ax.transAxes, fontsize=8, ha='right', va='top')
            
            ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f'{args.output}_cross.png', dpi=300, bbox_inches='tight')
    plt.savefig(f'{args.output}_cross.pdf', bbox_inches='tight')
    print(f"\nFigures saved: {args.output}_cross.png, .pdf")
    plt.close()
    
    # Plot correlation functions
    fig, axes = plt.subplots(n_top, n_top, figsize=(3*n_top, 3*n_top))
    
    t_corr_ps = t_corr / 1000  # Convert to ps
    
    for ii in range(n_top):
        for jj in range(n_top):
            ax = axes[ii, jj]
            C = cross_corr_matrix[ii, jj, :]
            
            if ii == jj:
                ax.plot(t_corr_ps, C, 'b-', linewidth=1)
            else:
                ax.plot(t_corr_ps, C, 'k-', linewidth=1)
                ax.axhline(y=0, color='gray', linestyle='-', linewidth=0.5)
            ...
            if jj == 0:
                ax.set_ylabel(f'{top_names[ii]}', fontsize=10)
            if ii == n_top - 1:
                ax.set_xlabel('t (ps)', fontsize=9)
            
            ax.grid(True, alpha=0.3)
    
    plt.suptitle('Cross-correlation functions C(t)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{args.output}_cross_corr.png', dpi=300, bbox_inches='tight')
    plt.savefig(f'{args.output}_cross_corr.pdf', bbox_inches='tight')
    print(f"Figures saved: {args.output}_cross_corr.png, .pdf")
    plt.close()
    
    # Save cross-correlation data
    with open(f'{args.output}_cross.dat', 'w') as f:
        f.write("# Cross-correlation spectral density analysis\n")
        f.write(f"# Input: {args.input}\n")
        f.write(f"# Temperature: {T} K\n")
        f.write(f"# Top {n_top} molecules by λ: {', '.join(top_names)}\n")
        f.write("#\n")
        f.write("# Cross-correlation λ matrix (eV):\n")
        f.write(f"# {'':>10}")
        for name in top_names:
            f.write(f" {name:>12}")
        f.write("\n")
        for ii, name_i in enumerate(top_names):
            f.write(f"# {name_i:>10}")
            for jj in range(n_top):
                f.write(f" {lambda_cross_matrix[ii, jj]*cm_to_eV:>12.4f}")
            f.write("\n")
        f.write("#\n")
        
        # Header for spectral density data
        f.write("# nu(cm-1)")
        for ii in range(n_top):
            for jj in range(n_top):
                f.write(f"  J_{top_names[ii]}_{top_names[jj]}")
        f.write("\n")
        
        for k in range(len(nu_out)):
            f.write(f"{nu_out[k]:10.2f}")
            for ii in range(n_top):
                for jj in range(n_top):
                    f.write(f" {J_cross_matrix[ii, jj, k]:14.6e}")
            f.write("\n")
    print(f"Data saved: {args.output}_cross.dat")
