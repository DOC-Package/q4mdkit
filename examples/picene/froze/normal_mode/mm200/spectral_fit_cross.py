#!/usr/bin/env python3
"""
Spectral domain fitting using quadratic form optimization (mm200):
J_mol200(ω) = κ^T J_qq(ω) κ

Optimization: minimize ||J_mol200(ω) - κ^T J_qq(ω) κ||²

J_qq includes:
- Auto-correlations: J_qq,kk(ω) from q_k(t)
- Cross-correlations: J_qq,kl(ω) from q_k(t) and q_l(t) (asymmetric)

Target: J_mol200 - Electrostatic energy contribution from molecule 200
Caches computed spectral densities for faster re-runs.
Mode range: 150-1750 cm⁻¹
Fit range: 200-1750 cm⁻¹
"""
import numpy as np
...
print("="*70)
print("Spectral Fitting: Quadratic Form Optimization (mm200)")
print("Minimize ||J_mol200(ω) - κ^T J_qq(ω) κ||²")
print("="*70)

# Unit conversion
cm_to_meV = 0.123984  # meV/cm⁻¹

# Load J_mol200 from spectral_density.dat
print("\nLoading J_mol200 from spectral_density.dat...")
spectral_data = np.loadtxt('spectral_density.dat')
freq_data = spectral_data[:, 0]  # nu (cm⁻¹)
J_target_data = spectral_data[:, 3]  # J_mol200 (cm⁻¹) - column index 3
...
# Filter modes for fitting: 150 ≤ freq ≤ 1750 cm⁻¹
fit_mask = (mode_freqs_all >= 150) & (mode_freqs_all <= 1750)
mode_indices = mode_indices_all[fit_mask]
mode_freqs = mode_freqs_all[fit_mask]
print(f"\nFiltered to {len(mode_indices)} modes with 150 ≤ freq ≤ 1750 cm⁻¹")
...
# Frequency grid (200-1750 cm⁻¹)
freq_grid = np.linspace(200, 1750, 1551)
print(f"\nFrequency grid: {len(freq_grid)} points (200-1750 cm⁻¹)")

# Interpolate J_mol200 to the frequency grid
from scipy.interpolate import interp1d
J_target_interp = interp1d(freq_data, J_target_data, kind='linear', fill_value='extrapolate')
J_target = J_target_interp(freq_grid)
print(f"  Interpolated J_mol200 to fitting grid")

# ============================================================================
# Load or compute J_qq matrix (J_mol200 already loaded above)
# ============================================================================
# Cache file name includes segmentation parameters to invalidate old caches
cache_file_J_qq = f'spectral_cache_J_qq_mm200_seg{int(segment_ps)}ps_corr{int(corr_ps)}ps.npz'

# Check if cache exists and has compatible size
compute_J_qq = True

J_target_integral = np.trapezoid(J_target, freq_grid)
print(f"  ∫J_mol200 = {J_target_integral:.6e} cm⁻¹²")
...
print(f"\nOriginal scales (RMS):")
print(f"  J_mol200 RMS: {J_target_rms:.6e}")
print(f"  J_qq RMS: {J_qq_rms:.6e}")
print(f"  Ratio (J_mol200/J_qq): {J_target_rms / J_qq_rms:.6e}")
...
print(f"  J_mol200_normalized RMS: {np.sqrt(np.mean(J_target_normalized**2)):.6f}")
print(f"  J_qq_normalized RMS: {np.sqrt(np.mean(J_qq_normalized**2)):.6f}")

# ============================================================================
# Optimization: minimize ||J_mol200(ω) - κ^T J_qq(ω) κ||² + λ||κ||²
# ============================================================================
print("\n" + "="*70)
print("Optimizing κ to minimize ||J_mol200 - κ^T J_qq κ||² + λ||κ||²")
print("="*70)
...
print(f"  Using normalized J_mol200 and J_qq")
...
with open('g_k_quadratic_form.dat', 'w') as f:
    f.write(f"# Coefficients from Quadratic Form fit (mm200)\n")
    f.write(f"# J(ω) = κ^T J_qq(ω) κ\n")
    f.write(f"# Fitted with normalized J_mol200 and J_qq\n")
    f.write(f"# Normalization:\n")
    f.write(f"#   Common scale factor = {scale_factor:.6e}\n")
    f.write(f"#   (Both J_mol200 and J_qq normalized by J_target_rms)\n")
    f.write(f"# R² = {R2:.6f}\n")
    f.write(f"# Capture = {capture:.2f}%\n")
    f.write(f"# Fitting range: {freq_grid[0]:.0f}-{freq_grid[-1]:.0f} cm⁻¹\n")
    f.write(f"# Mode range: 150-1750 cm⁻¹\n")
    f.write(f"#\n")
...
with open('spectral_density_quadratic_form.dat', 'w') as f:
    f.write("# Frequency(cm⁻¹)  J_mol200(cm⁻¹²)  J_model(cm⁻¹²)  J_diag(cm⁻¹²)  J_offdiag(cm⁻¹²)  Residual(cm⁻¹²)\n")
...
print(f"  Method: minimize ||J_mol200(ω) - κ^T J_qq(ω) κ||²")
print(f"  Normalization: Common scale factor for numerical stability")
print(f"    Scale factor = {scale_factor:.6e}")
print(f"    (Both J_mol200 and J_qq normalized by J_target_rms)")
print(f"  R² = {R2:.4f}")
...
print(f"    ✓ Theoretically exact for linear model e(t) = κ^T q(t)")
...
ax = axes[0]
ax.plot(freq_grid, J_target * cm_to_meV, 'k-', linewidth=2, label='Elstat (mol200)')
...
y_max = max(np.max(J_target), np.max(J_model)) * cm_to_meV * 1.28
...
plt.tight_layout()
plt.savefig('spectral_fit_mm200.png', dpi=300, bbox_inches='tight')
plt.savefig('spectral_fit_mm200.pdf', bbox_inches='tight')
print("  Saved: spectral_fit_mm200.png, .pdf")
plt.close()
