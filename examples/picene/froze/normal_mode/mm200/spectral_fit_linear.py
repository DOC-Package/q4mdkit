...
# Read energy_diff.dat header to find mol200 column
...
if mol200_col is None:
    raise ValueError("mol200 column not found in energy_diff.dat")

print(f"  mol200 column index: {mol200_col}")

energy_diff_data = np.loadtxt('energy_diff.dat')
energy_diff = energy_diff_data[:, mol200_col]  # mol200のエネルギー差 (eV)

# Interpolate NaN values in energy_diff
energy_diff = interpolate_nan(energy_diff, "energy_diff")

# Load mode information (mol200用)
mode_indices_all = []
mode_freqs_all = []
with open('mode_coords_mm200_stats.txt') as f:
    for line in f:
        line = line.strip()
...
# Load mode coordinates (mol200用)
print("  Loading mode_coords_mm200.txt...")
normalized_data = np.loadtxt('mode_coords_mm200.txt')
