...
mode_indices_all = []
mode_freqs_all = []
with open('mode_coords_pc_stats.txt') as f:
    for line in f:
        line = line.strip()
...
# Load mode coordinates from mode_coords_pc.txt
q_k_data = np.loadtxt('mode_coords_pc.txt')
q_k_all = q_k_data[:, 1:]
...
ax1.plot(freq_grid, J_linear, 'r--', label='J_linear (Stage 1)', linewidth=1.5)
ax1.set_xlabel('Frequency (cm⁻¹)')
ax1.set_ylabel('J(ω) (cm⁻¹²)')
ax1.set_title(f'Stage 1: Linear Fit (R² = {R2_linear:.4f})')
ax1.legend()
ax1.grid(True, alpha=0.3)
ax1.set_xlim(freq_grid[0], freq_grid[-1])

# Plot 2: Residual after Stage 1
ax2 = axes[0, 1]
ax2.plot(freq_grid, J_residual_1, 'k-', linewidth=1)
ax2.axhline(0, color='r', linestyle='--', linewidth=0.5)
ax2.fill_between(freq_grid, J_residual_1, 0, alpha=0.3, color='gray')
ax2.set_xlabel('Frequency (cm⁻¹)')
ax2.set_ylabel('Residual (cm⁻¹²)')
ax2.set_title(f'Residual: J_total - J_linear ({100*J_residual_1_integral/J_total_integral:.1f}% of total)')
ax2.grid(True, alpha=0.3)
ax2.set_xlim(freq_grid[0], freq_grid[-1])

# Plot 3: c_k coefficients vs mode frequency
ax3 = axes[1, 0]
active_mask = c_k_physical > 1e-10
ax3.bar(mode_freqs[active_mask], c_k_physical[active_mask], width=15, color='green', alpha=0.7, edgecolor='black', linewidth=0.5)
ax3.set_xlabel('Mode Frequency (cm⁻¹)')
ax3.set_ylabel('c_k')
ax3.set_title(f'Linear Coefficients c_k ({n_active} active modes)')
ax3.grid(True, alpha=0.3, axis='y')

# Plot 4: Top contributing modes
ax4 = axes[1, 1]
top_n = min(15, n_active)
top_idx = np.argsort(c_k_physical)[-top_n:][::-1]
bars = ax4.barh(range(top_n), c_k_physical[top_idx], color='green', alpha=0.7, edgecolor='black')
ax4.set_yticks(range(top_n))
ax4.set_yticklabels([f'Mode {mode_indices[i]} ({mode_freqs[i]:.0f} cm⁻¹)' for i in top_idx])
ax4.set_xlabel('c_k')
ax4.set_title(f'Top {top_n} Modes by c_k')
ax4.invert_yaxis()
ax4.grid(True, alpha=0.3, axis='x')

plt.tight_layout()
plt.savefig('spectral_fit_stage1.png', dpi=150, bbox_inches='tight')
print(f"Saved: spectral_fit_stage1.png")
plt.close()
