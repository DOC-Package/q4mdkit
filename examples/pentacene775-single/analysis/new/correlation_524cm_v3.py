#!/usr/bin/env python3
"""
524 cm⁻¹ピークに寄与する3次・4次相関の解析（v3）

v2からの変更点:
- 524 cm⁻¹周辺のモード（26, 27, 28, 29）を含む組み合わせを追加
  - Mode 26: 497.1 cm⁻¹
  - Mode 27: 516.0 cm⁻¹  
  - Mode 28: 530.2 cm⁻¹
  - Mode 29: 575.4 cm⁻¹
...
# キャッシュファイル
CACHE_FILE = 'correlation_524cm_v3_cache.pkl'

# 524 cm⁻¹周辺のモード（重点探索）
CENTRAL_MODES = [26, 27, 28, 29]  # 497, 516, 530, 575 cm⁻¹
...
sys.stdout = Logger('correlation_524cm_v3.log')
...
def find_target_combinations(mode_coords, mode_info, freq_min=200, freq_max=1600):
    ...
    524 cm⁻¹周辺のモード(26,27,28,29)を含む組み合わせも追加
    t=0の値でソートして上位を返す
    """
    ...
    print(f"\n524周辺の重点探索モード: {CENTRAL_MODES}")
    for m in CENTRAL_MODES:
        print(f"  Mode {m}: {mode_info[m]:.1f} cm⁻¹")
    print()
    
    # 3次相関の候補を収集（t=0の値で評価）
    print("3次相関の候補を収集中...")
    candidates_3rd = []
    added_3rd = set()  # 重複チェック
    ...
    for i, freq_i in valid_modes:
        ...
        for j, freq_j in valid_modes:
            ...
            for k, freq_k in valid_modes:
                ...
                # 524周辺モードを含むか
                if k in CENTRAL_MODES:
                    mechanisms.append(('central_k', freq_k))
                if i in CENTRAL_MODES or j in CENTRAL_MODES:
                    central_m = i if i in CENTRAL_MODES else j
                    mechanisms.append(('central_ij', mode_info[central_m]))
                
                if mechanisms:
                    key = (i, j, k)
                    if key not in added_3rd:
                        qk = mode_coords[:, k-7]
    ...
    print(f"3次相関の候補数: {len(candidates_3rd)}, 上位100を選択")
    ...
    for i, freq_i in valid_modes:
        ...
        for j, freq_j in valid_modes:
            ...
            if i in CENTRAL_MODES or j in CENTRAL_MODES:
                central_m = i if i in CENTRAL_MODES else j
                mechanisms.append(('central', mode_info[central_m]))
            
            if mechanisms:
                key = (i, j)
                if key not in added_4th:
    ...
    print(f"4次相関の候補数: {len(candidates_4th)}, 上位100を選択")
    
    return candidates_3rd[:100], candidates_4th[:100]
