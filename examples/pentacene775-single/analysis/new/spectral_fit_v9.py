#!/usr/bin/env python3
"""
v9: Stage 1の残差をモード14, 15, 16, 25, 26, 36, 40の3次・4次相関でフィット

Stage 1: Linear terms with NNLS (c_k ≥ 0) → g_k
Stage 2: モード14, 15, 16, 25, 26, 36, 40の3次・4次相関で非線形最小二乗フィット
...
print("="*70)
print("v9: Stage 1残差をモード14,15,16,25,26,36,40の3次・4次相関でフィット")
print("="*70)
...
# ============================================================================
# Stage 2: モード14,15,16,25,26,36,40の3次・4次相関をフィット
# ============================================================================
print("\n" + "="*70)
print("Stage 2: モード14,15,16,25,26,36,40の3次・4次相関をフィット")
print("="*70)
...
cache_file_v9 = 'spectral_cache_v9_4th.npz'

# 4次相関ペアのリスト (7モードの場合は28ペア: 7*8/2 = 28)
modes_list = [14, 15, 16, 25, 26, 36, 40]
pairs_4th = []
pair_names_4th = []
for i, mi in enumerate(modes_list):
    for mj in modes_list[i:]:
        pairs_4th.append((mi, mj))
        pair_names_4th.append(f'({mi},{mj})')

print(f"  4次相関ペア: {len(pairs_4th)}組")

q_dict = {14: q14, 15: q15, 16: q16, 25: q25, 26: q26, 36: q36, 40: q40}

if os.path.exists(cache_file_v9):
    print(f"Loading cached from {cache_file_v9}...")
    cache_v9 = np.load(cache_file_v9)
    if len(cache_v9['freq_grid']) != len(freq_grid):
        print("  Cache freq_grid mismatch, recomputing...")
        os.remove(cache_file_v9)
        cache_v9 = None
    else:
        J_4th = {}
        for i, j in pairs_4th:
            key = f'J_4th_{i}_{j}'
            J_4th[(i,j)] = cache_v9[key]
else:
    cache_v9 = None
...
if cache_v9 is None:
    ...
    np.savez_compressed(cache_file_v9, **save_dict)
    print(f"  Saved to {cache_file_v9}")
...
# ============================================================================
# Stage 2 フィッティング（4次: d_ij²）
# J_cross = Σ d_ij² * J_4th(i,j)
# フィットパラメータは d_ij のみ
# ============================================================================
print("\n" + "="*70)
print(f"Stage 2 フィッティング (d_ij: {len(pairs_4th)}個)")
print("="*70)
...
n_params = len(pairs_4th)  # d_ij のみ (28個)
...
ax4.plot(freq_grid, J_residual_1, 'g--', label='Stage 1 residual', linewidth=1, alpha=0.7)
ax4.axhline(0, color='gray', linestyle=':', linewidth=0.5)
ax4.set_xlabel('Frequency (cm⁻¹)')
ax4.set_ylabel('J(ω)')
ax4.set_title('4th order contribution vs Stage 1 residual')
...
plt.tight_layout()
plt.savefig('spectral_fit_v9.png', dpi=150, bbox_inches='tight')
print("Saved: spectral_fit_v9.png")
plt.close()
...
for (i,j), name in zip(pairs_4th, pair_names_4th):
    ...
    if np.abs(val_4th) > 1e-10:
        print(f"    {name}: 4th={val_4th:+.6e}")
...
# 線形係数 g_k の出力
coeff_linear_file = 'coefficients_linear_v9.dat'
...
# 非線形係数 d_ij の出力
coeff_d_file = 'coefficients_d_v9.dat'
...
# サマリーファイルの出力
summary_file = 'fit_summary_v9.dat'
with open(summary_file, 'w') as f:
    f.write("# Spectral Fit v9 Summary\n")
    f.write(f"# Date: 2026-02-04\n")
    f.write(f"# Modes used for 4th order: 14, 15, 16, 25, 26, 36, 40\n")
    f.write("#\n")
    f.write("# Model: J = sum_k g_k^2 J_k + sum_{i,j} d_ij^2 J_4th(i,j)\n")
    f.write("#\n")
