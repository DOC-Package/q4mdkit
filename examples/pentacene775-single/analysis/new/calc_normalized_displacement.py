...
The normalized displacement is defined as:
    Δq̃_k(t) = Δq_k(t) / <Δq_k²>_t
where:
    Δq_k(t) = q_k(t) - <q_k>_t

This script reads mode_coords_new.txt and outputs normalized_displacement.txt
"""
...
def main():
    ...
    input_file = "mode_coords_new.txt"
    output_file = "normalized_displacement.txt"
    ...
    # Calculate the normalized displacement: Δq̃_k(t) = Δq_k(t) / <Δq_k²>_t
    # Note: Be careful with division by zero for modes with very small variance
    with np.errstate(divide='ignore', invalid='ignore'):
        normalized_disp = delta_q / variance_q  # shape: (n_frames, n_modes)
...
Definition: Δq̃_k(t) = Δq_k(t) / <Δq_k²>_t, where Δq_k(t) = q_k(t) - <q_k>_t
Frames: {n_frames}, Modes: {n_modes}
Mode indices: 7 to 108 (1-indexed)
Columns: frame_index, d̃_7, d̃_8, ..., d̃_3N"""
    ...
    stats_file = "normalized_displacement_stats.txt"
