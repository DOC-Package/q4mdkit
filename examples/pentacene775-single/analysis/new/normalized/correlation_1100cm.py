#!/usr/bin/env python3
"""
1050-1150 cm⁻¹ピークに寄与する3次・4次相関の解析（正規化変位版）
...
# キャッシュファイル
CACHE_FILE = 'correlation_1100cm_normalized_cache.pkl'
...
sys.stdout = Logger('correlation_1100cm_normalized.log')
    ...
    for i, freq_i in valid_modes:
        ...
        for j, freq_j in valid_modes:
            ...
            for k, freq_k in valid_modes:
                # 1050-1150 cm⁻¹に寄与する可能性があるか判定（和のみ、差は排除）
                mechanisms = []
...
def main():
    print("=" * 70)
    print(f"1050-1150 cm⁻¹ ピークに寄与する相関関数の解析（正規化変位版）")
    ...
    # 結果表示
    print("\n" + "=" * 70)
    print(f"3次相関: 1050-1150 cm⁻¹ 付近のピーク (正規化変位)")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print(f"4次相関: 1050-1150 cm⁻¹ 付近のピーク (正規化変位)")
    print("=" * 70)
    ...
    ax.set_title(f'3rd Order Correlation <Δq̃_i(t)Δq̃_j(t)Δq̃_k(0)>: Peak Heights at {TARGET_FREQ:.0f}±{TOLERANCE:.0f} cm⁻¹ (Normalized, Top 30)')
    ...
    ax.set_ylabel('Peak Height at 1050-1150 cm⁻¹')
    ax.set_title(f'4th Order Correlation <Δq̃_i(t)Δq̃_j(t)Δq̃_i(0)Δq̃_j(0)>: Peak Heights at {TARGET_FREQ:.0f}±{TOLERANCE:.0f} cm⁻¹ (Normalized, Top 30)')
    ax.axhline(y=0, color='k', linestyle='-', linewidth=0.5)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('correlation_1100cm_normalized_peaks.png', dpi=150)
    print("Saved: correlation_1100cm_normalized_peaks.png")
    plt.close()
    ...
    ax.set_title('3rd Order Correlations (Normalized, Top 30)')
    ...
    ax.set_xlabel('Frequency (cm⁻¹)')
    ax.set_ylabel('Spectral Density')
    ax.set_title('4th Order Correlations (Normalized, Top 30)')
    ax.legend(fontsize=7)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('correlation_1100cm_normalized_spectra.png', dpi=150)
    print("Saved: correlation_1100cm_normalized_spectra.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('correlation_1100cm_normalized_comparison.png', dpi=150)
    print("Saved: correlation_1100cm_normalized_comparison.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('correlation_1100cm_normalized_mechanisms.png', dpi=150)
    print("Saved: correlation_1100cm_normalized_mechanisms.png")
    plt.close()
