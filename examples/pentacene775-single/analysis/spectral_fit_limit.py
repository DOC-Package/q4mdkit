...
# Frequency grid (450-600 cm⁻¹)
freq_grid = np.linspace(450, 600, 151)
print(f"\nFrequency grid: {len(freq_grid)} points (450-600 cm⁻¹)")

# ============================================================================
# Load or compute J_qq matrix and J_total
# ============================================================================
cache_file_total = 'spectral_limit_cache_J_total.npy'
...
# Figure 1: Spectral density comparison
fig, axes = plt.subplots(2, 2, figsize=(12, 10))

# Plot 1a: J_total vs J_model
ax1 = axes[0, 0]
ax1.plot(freq_grid, J_total, 'b-', label='J_total (MD)', linewidth=1.5)
ax1.plot(freq_grid, J_model, 'r--', label='J_model (fit)', linewidth=1.5)
ax1.set_xlabel('Frequency (cm⁻¹)')
ax1.set_ylabel('J(ω) (cm⁻¹²)')
ax1.set_title(f'Spectral Density Fit (R² = {R2:.4f})')
ax1.legend()
ax1.grid(True, alpha=0.3)
ax1.set_xlim(freq_grid[0], freq_grid[-1])

# Plot 1b: Decomposition (diagonal vs off-diagonal)
ax2 = axes[0, 1]
ax2.plot(freq_grid, J_total, 'b-', label='J_total', linewidth=1.5, alpha=0.7)
ax2.plot(freq_grid, J_diag, 'g-', label='J_diag (auto)', linewidth=1.5)
ax2.plot(freq_grid, J_offdiag, 'm-', label='J_offdiag (cross)', linewidth=1.5)
ax2.axhline(0, color='k', linestyle='-', linewidth=0.5)
ax2.set_xlabel('Frequency (cm⁻¹)')
ax2.set_ylabel('J(ω) (cm⁻¹²)')
ax2.set_title('Decomposition: Diagonal vs Off-diagonal')
ax2.legend()
ax2.grid(True, alpha=0.3)
ax2.set_xlim(freq_grid[0], freq_grid[-1])

# Plot 1c: Residual
ax3 = axes[1, 0]
ax3.plot(freq_grid, residual, 'k-', linewidth=1)
ax3.axhline(0, color='r', linestyle='--', linewidth=0.5)
ax3.fill_between(freq_grid, residual, 0, alpha=0.3)
ax3.set_xlabel('Frequency (cm⁻¹)')
ax3.set_ylabel('Residual (cm⁻¹²)')
ax3.set_title('Residual (J_total - J_model)')
ax3.grid(True, alpha=0.3)
ax3.set_xlim(freq_grid[0], freq_grid[-1])

# Plot 1d: g_k coefficients vs mode frequency
ax4 = axes[1, 1]
colors = ['red' if g > 0 else 'blue' for g in g_opt]
ax4.bar(mode_freqs, g_opt, width=15, color=colors, alpha=0.7, edgecolor='black', linewidth=0.5)
ax4.axhline(0, color='k', linestyle='-', linewidth=0.5)
ax4.set_xlabel('Mode Frequency (cm⁻¹)')
ax4.set_ylabel('g_k')
ax4.set_title('Coupling Coefficients g_k')
ax4.grid(True, alpha=0.3, axis='y')

plt.tight_layout()
plt.savefig('spectral_fit_limit.png', dpi=150, bbox_inches='tight')
print(f"Saved: spectral_fit_limit.png")

# Figure 2: Top contributing modes
fig2, axes2 = plt.subplots(1, 2, figsize=(12, 5))

# Top 10 modes by |g_k|
top_n = min(10, N_modes)
top_idx_plot = np.argsort(np.abs(g_opt))[-top_n:][::-1]

ax5 = axes2[0]
bar_colors = ['red' if g_opt[i] > 0 else 'blue' for i in top_idx_plot]
bars = ax5.barh(range(top_n), np.abs(g_opt[top_idx_plot]), color=bar_colors, alpha=0.7, edgecolor='black')
ax5.set_yticks(range(top_n))
ax5.set_yticklabels([f'Mode {mode_indices[i]} ({mode_freqs[i]:.0f} cm⁻¹)' for i in top_idx_plot])
ax5.set_xlabel('|g_k|')
ax5.set_title(f'Top {top_n} Modes by |g_k|')
ax5.invert_yaxis()
ax5.grid(True, alpha=0.3, axis='x')

# Add legend for colors
from matplotlib.patches import Patch
legend_elements = [Patch(facecolor='red', alpha=0.7, label='Positive g_k'),
                   Patch(facecolor='blue', alpha=0.7, label='Negative g_k')]
ax5.legend(handles=legend_elements, loc='lower right')

# g_k^2 contribution (diagonal only)
ax6 = axes2[1]
g_sq = g_opt**2
g_sq_sorted_idx = np.argsort(g_sq)[-top_n:][::-1]
ax6.barh(range(top_n), g_sq[g_sq_sorted_idx], color='green', alpha=0.7, edgecolor='black')
ax6.set_yticks(range(top_n))
ax6.set_yticklabels([f'Mode {mode_indices[i]} ({mode_freqs[i]:.0f} cm⁻¹)' for i in g_sq_sorted_idx])
ax6.set_xlabel('g_k²')
ax6.set_title(f'Top {top_n} Modes by g_k² (diagonal weight)')
ax6.invert_yaxis()
ax6.grid(True, alpha=0.3, axis='x')

plt.tight_layout()
plt.savefig('spectral_fit_limit_modes.png', dpi=150, bbox_inches='tight')
print(f"Saved: spectral_fit_limit_modes.png")
