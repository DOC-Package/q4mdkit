...
q_k_all = q_k_data[:, 1:]
n_available_modes = q_k_all.shape[1]
print(f"  Available mode columns in data: {n_available_modes}")

# Determine valid mode indices based on available data (mode 7 -> column 0)
available_mode_indices = np.arange(7, 7 + n_available_modes)
print(f"  Available mode indices: {available_mode_indices[0]} to {available_mode_indices[-1]}")

# Filter to only use modes that exist in both stats file AND data file
valid_mask = np.isin(mode_indices, available_mode_indices)
if not np.any(valid_mask):
    raise ValueError(f"No modes in range 500-1900 cm⁻¹ are available in data file. "
                     f"Available modes: {available_mode_indices}, Requested modes: {mode_indices}")
mode_indices = mode_indices[valid_mask]
mode_freqs = mode_freqs[valid_mask]
print(f"  After filtering to available data: {len(mode_indices)} modes")
