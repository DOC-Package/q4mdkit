...
spectral_data = np.loadtxt('spectral_density_comparison.dat')
freq_data = spectral_data[:, 0]  # nu (cm⁻¹)
J_internal_meV = spectral_data[:, 3]  # J_internal (meV)
...
# Filter modes for fitting: 1100 ≤ freq ≤ 1750 cm⁻¹
fit_mask = (mode_freqs_all >= 1100) & (mode_freqs_all <= 1750)
mode_indices = mode_indices_all[fit_mask]
mode_freqs = mode_freqs_all[fit_mask]
print(f"\nFiltered to {len(mode_indices)} modes with 1100 ≤ freq ≤ 1750 cm⁻¹")
...
# Frequency grid (1100-1750 cm⁻¹)
freq_grid = np.linspace(1100, 1750, 651)
print(f"\nFrequency grid: {len(freq_grid)} points (1100-1750 cm⁻¹)")
...
# ============================================================================
cache_file_J_qq = 'spectral_cache_J_qq_internal.npz'
...
# ============================================================================
# Optimization: minimize ||J_internal(ω) - g^T J_qq(ω) g||² + λ||g||²
# ============================================================================
print("\n" + "="*70)
print("Optimizing g to minimize ||J_internal - g^T J_qq g||² + λ||g||²")
print("="*70)
...
with open('g_k_quadratic_form.dat', 'w') as f:
    ...
    f.write(f"# Mode range: 1100-1750 cm⁻¹\n")
    f.write(f"#\n")
...
# Right panel: Mode contributions (|g_k|²)
ax = axes[1]
g_sq = g_opt**2
# Sort by contribution
sort_idx = np.argsort(g_sq)[::-1]

# Bar plot with colors based on sign of g
...
ax.set_xlabel('Mode rank by |g|²', fontsize=16)
ax.set_ylabel('|g$_k$|²', fontsize=16)
...
ax.legend([plt.Rectangle((0,0),1,1,fc='C0',alpha=0.7), 
          ...
          ['g > 0', 'g < 0'], fontsize=16, loc='upper right')
