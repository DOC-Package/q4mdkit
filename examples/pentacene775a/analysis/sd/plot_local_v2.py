...
def compute_correlation_segmented(delta: np.ndarray, dt: float, 
                                   segment_length: int, corr_length: int,
                                   min_last_segment_frac: float = 0.5) -> tuple:
    """Compute correlation function with segment averaging.
    
    If the last segment has at least min_last_segment_frac * segment_length frames
    (and >= corr_length), it will be included.
    """
    N = len(delta)
    n_full_segments = N // segment_length
    remainder = N % segment_length
    
    if n_full_segments < 1 and remainder < corr_length:
        raise ValueError(f"Segment length ({segment_length}) > data length ({N})")
    
    # Include last partial segment if it's large enough
    min_last_len = max(int(segment_length * min_last_segment_frac), corr_length)
    include_last = (remainder >= min_last_len)
    n_segments = n_full_segments + (1 if include_last else 0)
...
def main():
    ...
    ax.plot(nu_out, SD_dE1 * cm_to_meV, color='#E57373', linestyle='-', linewidth=2, 
            label=f'$E_I$ (λ = {lambda_dE1*cm_to_meV:.2f} meV)')
    ax.plot(nu_out, SD_dE2 * cm_to_meV, color='#64B5F6', linestyle='--', linewidth=2, 
            label=f'$E_J$ (λ = {lambda_dE2*cm_to_meV:.2f} meV)')
    ax.plot(nu_out, SD_total * cm_to_meV, 'k:', linewidth=2.5, 
            label=f'Single (λ = {lambda_total*cm_to_meV:.2f} meV)')
    ...
    ax.plot(t_corr / 1000, C_dE1 * cm2_to_meV2, color='#E57373', linestyle='-', linewidth=2, label='$E_I$')
    ax.plot(t_corr / 1000, C_dE2 * cm2_to_meV2, color='#64B5F6', linestyle='--', linewidth=2, label='$E_J$')
    ax.plot(t_corr / 1000, C_total * cm2_to_meV2, 'k:', linewidth=2.5, label='Single')
    ...
    # Spectral densities
    ax = axes[1]
    ax.plot(nu_out, SD_dE1 * cm_to_meV, color='#E57373', linestyle='-', linewidth=2, label='$E_I$')
    ax.plot(nu_out, SD_dE2 * cm_to_meV, color='#64B5F6', linestyle='--', linewidth=2, label='$E_J$')
    ax.plot(nu_out, SD_total * cm_to_meV, 'k:', linewidth=2.5, label='Single')
    ...
    ax_inset = ax.inset_axes([0.09, 0.43, 0.55, 0.52])  # [x, y, width, height] in axes coordinates
    nu_max_inset = 300  # cm⁻¹
    ax_inset.plot(nu_out, SD_dE1 * cm_to_meV, color='#E57373', linestyle='-', linewidth=1.5)
    ax_inset.plot(nu_out, SD_dE2 * cm_to_meV, color='#64B5F6', linestyle='--', linewidth=1.5)
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("# nu(cm-1)  SD_EI(meV)  SD_EJ(meV)  SD_total(meV)\n")
        for i in range(len(nu_out)):
            f.write(f"{nu_out[i]:10.2f} {SD_dE1[i]*cm_to_meV:14.6e} {SD_dE2[i]*cm_to_meV:14.6e} {SD_total[i]*cm_to_meV:14.6e}\n")
