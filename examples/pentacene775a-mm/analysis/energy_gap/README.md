...
```bash
bash examples/pentacene775a-mm/analysis/energy_gap/prepare_cation.sh --dry-run
```

Run it with the defaults for a +1 doublet on one-based residue 156:

```bash
bash examples/pentacene775a-mm/analysis/energy_gap/prepare_cation.sh
```

This creates `pentacene_cation.mol2` and `pentacene_cation.prmtop` in the input
...
```bash
bash examples/pentacene775a-mm/analysis/energy_gap/run_energy_gap.sh --dry-run
```

Run a short ten-frame check:

```bash
bash examples/pentacene775a-mm/analysis/energy_gap/run_energy_gap.sh \
    --stop 10 \
    --output /tmp/pentacene_energy_gap_test.csv
```

For the full saved trajectory, omit `--stop` (the default output is
`nve-mm/output/energy_gap.csv`). Use `--stride N` for a pilot calculation and
...
The current `qmatoms` selection contains zero-based atoms 5580--5615. This is
Amber/ParmEd residue 156 in the script's one-based `--resid` convention.

```bash
conda run -n parmed python \
    examples/pentacene775a-mm/analysis/energy_gap/prepare_charged_topology.py \
    --prmtop examples/pentacene775a-mm/input/pentacene.prmtop \
    --charged-mol2 target_charged.mol2 \
    --resid 156 \
    --expected-delta-charge 1 \
    --output examples/pentacene775a-mm/analysis/energy_gap/pentacene_cation.prmtop
```
...
optional and is not hard-coded to +1.

## 3. Validate an existing topology pair

```bash
conda run -n parmed python \
    examples/pentacene775a-mm/analysis/energy_gap/validate_topologies.py \
    --neutral-prmtop examples/pentacene775a-mm/input/pentacene.prmtop \
    --charged-prmtop examples/pentacene775a-mm/analysis/energy_gap/pentacene_cation.prmtop \
    --resid 156 \
    --expected-delta-charge 1
```
...
The DCD reader does not recover the physical timestamps. From `nve.csv`, frame
0 is at 600.004 ps and the saved-frame interval is 0.004 ps, so both time options
are specified explicitly:

```bash
conda run -n myash python \
    examples/pentacene775a-mm/analysis/energy_gap/compute_energy_gap.py \
    --neutral-prmtop examples/pentacene775a-mm/input/pentacene.prmtop \
    --charged-prmtop examples/pentacene775a-mm/input/pentacene.prmtop \
    --trajectory examples/pentacene775a-mm/nve-mm/output/nve.dcd \
    --resid 156 \
    --start 0 \
    --stop 10 \
    --time-origin-ps 600.004 \
    --time-step-ps 0.004 \
    --platform CPU \
    --threads 1 \
