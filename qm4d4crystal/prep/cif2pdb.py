...
def load_cif_file(cif_file: str) -> Atoms:
    """
    Load CIF file handling various edge cases.
    
    Some CIF files (like pentacene PENCEN01) contain explicit coordinates 
    for all atoms in the unit cell, including those generated by symmetry operations.
    ASE's default CIF reader applies space group symmetry operations, which can 
    cause issues (duplicate atoms or missing atoms) when the symmetry-equivalent 
    atoms are already explicitly listed in the CIF.
    
    This function tries multiple approaches:
    1. Read CIF treating it as P1 (no symmetry operations) 
    2. Fall back to standard ASE read
    3. Fall back to pymatgen if ASE fails
    ...
    # First, try reading CIF as P1 (ignoring symmetry operations)
    # This is needed for CIF files that already contain symmetry-expanded coordinates
    try:
        # Read and modify CIF to use P1 space group
        ...
        if has_sym_equiv:
            ...
            symmetry_written = False
            
            i = 0
            while i < len(lines):
                line = lines[i]
                
                # Replace space group with P1
                if '_symmetry_space_group_name_H-M' in line:
                    new_lines.append("_symmetry_space_group_name_H-M   'P 1'")
                    i += 1
                    continue
                ...
                if '_space_group_IT_number' in line:
                    new_lines.append("_space_group_IT_number           1")
                    i += 1
                    continue
                if '_space_group_name_H-M_alt' in line:
                    new_lines.append("_space_group_name_H-M_alt        'P 1'")
                    i += 1
                    continue
                
                # Detect symmetry operations loop
                if 'loop_' in line:
                    # Check if next line is symmetry-related
                    if i + 1 < len(lines) and '_symmetry_equiv_pos' in lines[i + 1]:
                        # Skip this loop and write P1 symmetry
                        if not symmetry_written:
                            new_lines.append('loop_')
                            new_lines.append('_symmetry_equiv_pos_site_id')
                            new_lines.append('_symmetry_equiv_pos_as_xyz')
                            new_lines.append('1 x,y,z')
                            symmetry_written = True
                        # Skip until end of symmetry loop
                        i += 1
                        while i < len(lines):
                            next_line = lines[i].strip()
                            if not next_line or next_line.startswith('_') or next_line.startswith('loop'):
                                if next_line.startswith('loop'):
                                    i -= 1  # Will be processed in next iteration
                                break
                            i += 1
                        i += 1
                        continue
            ...
            try:
                with warnings.catch_warnings():
                    ...
                    atoms = read(tmp_path)
                print(f"[INFO] Loaded CIF as P1 (symmetry-expanded): {len(atoms)} atoms")
            finally:
                os.unlink(tmp_path)
        
    except Exception as e1:
        import traceback
        print(f"[DEBUG] P1 conversion failed: {e1}")
        traceback.print_exc()
    
    # If that didn't work, try standard ASE read
    if atoms is None:
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                atoms = read(cif_file)
            print(f"[INFO] Loaded CIF with ASE (standard): {len(atoms)} atoms")
        except Exception as e2:
            print(f"[DEBUG] Standard ASE read failed: {e2}")
    
    # Fall back to pymatgen if ASE fails
    if atoms is None:
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
            ...
            print(f"[INFO] Loaded CIF with pymatgen: {len(atoms)} atoms")
        except Exception as e3:
            raise Exception(f"[ERROR] Could not load CIF file with any method: {e3}")
...
def cif_to_pdb(cif_file: str, output_pdb: str = None, output_mol2: str = None,
    ...
    # Load CIF file using helper that handles symmetry-expanded CIFs
    atoms = load_cif_file(cif_file)
